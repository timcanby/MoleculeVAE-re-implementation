...
def train(epochs, train_set_final, valid_set_final, logger, params: dict,
          device):
    model.train()
    train_loss = 0
    valid_loss = 0
                ...
                with torch.no_grad():
                    for data in valid_set_final:
                        ...
                        valid_loss += calculate_loss_by_type(valid_output, valid_smidata, valid_mean, valid_logvar,
                                                             valid_pre_loss, epochs, 'valid', logger)
...
def test(test_dataloader, epochs, logger, params: dict,
         device):
    test_loss = 0
    with torch.no_grad():
        for data in test_dataloader:
            ...
            # performance on separate prediction tasks [epoch,'logP_pre', 'qed_pre', 'SAS_pre', 'mae_logP', 'mae_qed', 'mae_SAS']
            logger.add_separate_prediction([epochs, torch.mean((test_pre[:, 0])).cpu().detach().numpy(),
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss, valid_loss = train(
            ...
            training_data_dataloader['valid_set_dataloader'],
            logger,
            params=params,
            device=device)
        test_loss = test(
            training_data_dataloader['test_dataloader'],
            epoch,
            logger,
            params=params,
            device=device)
