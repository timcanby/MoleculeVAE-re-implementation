...
def extract_properties_value(smile_onehot, smile_string):
    ...
    if os.path.exists('properties_value.json'):
        with open('properties_value.json') as json_file:
            ...
            for x, property in zip(smile_onehot, smile_string):
                try:
                    y=properties_dic[x]
                except:
                    y = caculate_target_values(property)
                    properties_dic[x]=y
                X_final.append(x)
                Y_values.append(y)
    else:
        for x, s_string in zip(smile_onehot, smile_string):
            ...
            X_final.append(x)
            Y_values.append(y)
            properties_dic = dict(zip(X_final, Y_values))
    '''creat & update properties_value.json'''
    with open('properties_value.json', "w") as outfile:
        json.dump(str(properties_dic.cpu().detach().numpy()), outfile)
